...
class LitebitProExchange(ExchangeBase):
    """
    LitebitProExchange connects with litebit.eu exchange and provides order book pricing, user account tracking and
        ...
        while True:
            try:
                ...
                self.logger().network(f"Unexpected error while fetching trading rules. Error: {str(e)}",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from litebit.eu. "
    ...
    def _format_trading_rules(self, instruments_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for rule in instruments_info["result"]["instruments"]:
            try:
                trading_pair = litebit_pro_utils.convert_from_exchange_trading_pair(rule["instrument_name"])
    ...
    async def _api_request(self,
        ...
        if is_auth_required:
            request_id = litebit_pro_utils.RequestId.generate_request_id()
            data = {"params": params}
            params = self._litebit_pro_auth.generate_auth_dict(path_url, request_id,
                                                               litebit_pro_utils.get_ms_timestamp(), data)
            headers = self._litebit_pro_auth.get_headers()
        else:
    ...
    async def _create_order(self,
        ...
        api_params = {"instrument_name": litebit_pro_utils.convert_to_exchange_trading_pair(trading_pair),
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} {order_type.name} order to litebit.eu for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> str:
        ...
        try:
            ...
            await self._api_request(
                ...
                {"instrument_name": litebit_pro_utils.convert_to_exchange_trading_pair(trading_pair),
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from LitebitPro. "
    ...
    def _process_order_message(self, order_msg: Dict[str, Any]):
        ...
        if tracked_order.is_cancelled:
            ...
            self.logger().info(f"The market order {client_order_id} has failed according to order status API. "
                               f"Reason: {litebit_pro_utils.get_api_reason(order_msg['reason'])}")
            self.trigger_event(MarketEvent.OrderFailure,
                               MarketOrderFailureEvent(
                                   self.current_timestamp,
                                   client_order_id,
    ...
    async def cancel_all(self, timeout_seconds: float):
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel all orders on LitebitPro. Check API key and network connection."
            )
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from LitebitPro. Check API key and network connection."
                )
    ...
    async def get_open_orders(self) -> List[OpenOrder]:
        ...
        for order in result["result"]["order_list"]:
            if litebit_pro_utils.HBOT_BROKER_ID not in order["client_oid"]:
                continue
            if order["type"] != "LIMIT":
            ...
            ret_val.append(
                OpenOrder(
                    ...
                    trading_pair=litebit_pro_utils.convert_from_exchange_trading_pair(order["instrument_name"]),
