...
class LitebitProAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_active_exchange_markets(cls) -> (pd.DataFrame):
        ...
        * quoteAsset
        """
        # Fetch raw exchange and markets data from LitebitPro
    ...
    async def get_exchange_markets_data(cls) -> (List):
        """
        Fetch LitebitPro exchange data from '/products' with following structure:
        ...
        async with aiohttp.ClientSession() as client:
            ...
            if exchange_markets_response.status != 200:
                raise IOError(f"Error fetching LitebitPro markets information. "
    ...
    async def get_trading_pairs(self) -> List[str]:
        ...
        for downstream reference since LitebitPro API uses id instead of trading
    ...
    async def get_snapshot(self, client: aiohttp.ClientSession, trading_pair: str, full: int = 1) -> Dict[str, Any]:
        ...
        async with client.get(Constants.GET_SNAPSHOT_URL.format(id=product_id, full=full)) as response:
            ...
            if response.status != 200:
                raise IOError(f"Error fetching LitebitPro market snapshot for {id}. "
    ...
    async def get_tracking_pairs(self) -> Dict[str, LitebitProOrderBookTrackerEntry]:
        """
        ...
        async with aiohttp.ClientSession() as client:
            ...
            retval: Dict[str, LitebitProOrderBookTrackerEntry] = {}
            ...
            for index, trading_pair in enumerate(trading_pairs):
                ...
                try:
                    ...
                    retval[trading_pair] = LitebitProOrderBookTrackerEntry(trading_pair, snapshot_timestamp, order_book)

                    self.logger().info(f"Initialized order book for {trading_pair}. "
                                       f"{index+1}/{number_of_pairs} completed")
                    # Each 1000 limit snapshot costs ?? requests and LitebitPro rate limit is ?? requests per second.
                ...
                async with websockets.connect(Constants.BAEE_WS_URL) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        ...
                        if event_type == 'updated':
                            ...
                            msg: OrderBookMessage = LitebitProOrderBook.diff_message_from_exchange(
                                diff_msg,
                                diff_timestamp,
                                metadata={
                                    "trading_pair": trading_pair,
                            ...
                            raise ValueError(f"Litebit Websocket message does not contain an event type - {diff_msg}")
