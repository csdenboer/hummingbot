...
class LitebitProAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def fetch_trading_pairs() -> List[str]:
        async with aiohttp.ClientSession() as client:
            async with client.get(f"{constants.REST_URL}{constants.GET_MARKETS_PATH}", timeout=10) as response:
    ...
    async def get_last_traded_prices(cls, trading_pairs: List[str]) -> Dict[str, float]:
        result = {}
        async with aiohttp.ClientSession() as client:
            resp = await client.get(f"{constants.REST_URL}{constants.GET_TICKERS_PATH}")
    ...
    async def _get_order_book_data(trading_pair: str) -> Dict[str, any]:
        ...
        async with aiohttp.ClientSession() as client:
            order_book_response = await client.get(
                f"{constants.REST_URL}{constants.GET_BOOK_PATH}",
