...
from .litebit_pro_utils import ms_timestamp_to_s
        ...
        async with aiohttp.ClientSession() as client:
            ...
            for t_pair in trading_pairs:
                last_trade = [o["a"] for o in resp_json["result"]["data"] if o["i"] ==
                              litebit_pro_utils.convert_to_exchange_trading_pair(t_pair)]
        ...
        async with aiohttp.ClientSession() as client:
            async with client.get(f"{constants.REST_URL}/public/get-ticker", timeout=10) as response:
                if response.status == 200:
                    from hummingbot.connector.exchange.litebit_pro.litebit_pro_utils import \
    ...
    async def get_order_book_data(trading_pair: str) -> Dict[str, any]:
        ...
        async with aiohttp.ClientSession() as client:
            orderbook_response = await client.get(
                ...
                f"{litebit_pro_utils.convert_to_exchange_trading_pair(trading_pair)}"
            )
        ...
        while True:
            try:
                ...
                await ws.subscribe(list(map(
                    lambda pair: f"trade.{litebit_pro_utils.convert_to_exchange_trading_pair(pair)}",
                    self._trading_pairs
                )))
                ...
                async for response in ws.on_message():
                    ...
                    for trade in response["result"]["data"]:
                        ...
                        trade_msg: OrderBookMessage = LitebitProOrderBook.trade_message_from_exchange(
                            trade,
                            trade_timestamp,
                            metadata={"trading_pair": litebit_pro_utils.convert_from_exchange_trading_pair(trade["i"])}
                        )
    ...
    async def listen_for_order_book_diffs(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        while True:
            try:
                ...
                await ws.subscribe(list(map(
                    lambda pair: f"book.{litebit_pro_utils.convert_to_exchange_trading_pair(pair)}.150",
                    self._trading_pairs
                )))
                ...
                async for response in ws.on_message():
                    ...
                    # LitebitPro does not offer order book diff ws updates.
                    orderbook_msg: OrderBookMessage = LitebitProOrderBook.snapshot_message_from_exchange(
                        order_book_data,
                        timestamp,
                        metadata={"trading_pair": litebit_pro_utils.convert_from_exchange_trading_pair(
