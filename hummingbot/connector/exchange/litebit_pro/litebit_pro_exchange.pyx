...
from hummingbot.connector.exchange.litebit_pro.litebit_pro_in_flight_order cimport LitebitProInFlightOrder
...
cdef class LitebitProExchangeTransactionTracker(TransactionTracker):
    cdef:
        LitebitProExchange _owner

    def __init__(self, owner: LitebitProExchange):
        super().__init__()
        self._owner = owner
...
cdef class LitebitProExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._user_stream_tracker = LitebitProUserStreamTracker(litebit_pro_auth=self._litebit_pro_auth, trading_pairs=trading_pairs)
        ...
        self._tx_tracker = LitebitProExchangeTransactionTracker(self)
        self._trading_rules = {}
    ...
    @property
    def litebit_pro_auth(self) -> LitebitProAuth:
        """
        :return: LitebitProAuth class (This is unique to LitebitPro market).
        """
        return self._litebit_pro_auth
    ...
    async def get_active_exchange_markets(self) -> pd.DataFrame:
        ...
        *required
        ...
        """
        return await LitebitProAPIOrderBookDataSource.get_active_exchange_markets()
    ...
    async def _api_request(self,
        ...
        """
        A wrapper for submitting API requests to LitebitPro
        ...
        headers = self.litebit_pro_auth.get_headers(path_url)
    ...
    cdef object c_get_fee(self,
        ...
        *required
        ...
        if order_type is OrderType.LIMIT and fee_overrides_config_map["litebit_pro_maker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["litebit_pro_maker_fee"].value / Decimal("100"))
        if order_type is OrderType.MARKET and fee_overrides_config_map["litebit_pro_taker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["litebit_pro_taker_fee"].value / Decimal("100"))
        return TradeFee(percent=maker_fee if order_type is OrderType.LIMIT else taker_fee)
        """
        is_maker = order_type is OrderType.LIMIT_MAKER
        return estimate_fee("litebit_pro", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) <= 0:
            ...
            products = LitebitProAPIOrderBookDataSource.reformat_trading_pairs(products)
        ...
        for product in products:
            try:
                ...
                retval.append(TradingRule(trading_pair,
                                          ...
                                          max_order_size=Decimal(sys.maxsize),  # LitebitPro doesn't specify max order size
                                          supports_market_orders=None))  # Not sure if LitebitPro has equivalent info
            except Exception:
            ...
            if order_update is None:
                try:
                    ...
                    if "order not found" in str(e).lower():
                        ...
                        self.logger().info(
                            f"The tracked order {client_order_id} does not exist on LitebitPro."
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                if execute_amount_diff > s_decimal_0:
                    self.logger().info(f"Filled {execute_amount_diff} out of {tracked_order.amount} of the "
                                       f"{order_type_description} order {tracked_order.client_order_id} according to LitebitPro user stream.")
                ...
                if event_status == "filled":
                    ...
                    if tracked_order.trade_type == TradeType.BUY:
                        self.logger().info(f"The market buy order {tracked_order.client_order_id} has completed "
                                           f"according to LitebitPro user stream.")
                        self.c_trigger_event(self.MARKET_BUY_ORDER_COMPLETED_EVENT_TAG,
                                             BuyOrderCompletedEvent(self._current_timestamp,
                                                                    tracked_order.client_order_id,
                                                                    tracked_order.base_asset,
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy {order_type_str} order to LitebitPro for "
                f"{decimal_amount} {trading_pair} {price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit buy order to LitebitPro. "
                                f"Check API key and network connection.{e}"
            )
    ...
    async def execute_sell(self,
        ...
        try:
            ...
            order_type_str = order_type.name.lower()
            self.logger().network(
                f"Error submitting sell {order_type_str} order to LitebitPro for "
                f"{decimal_amount} {trading_pair} {price}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit sell order to LitebitPro."
                                f"Check API key and network connection.{e}"
            )
            self.c_trigger_event(self.MARKET_ORDER_FAILURE_EVENT_TAG,
                                 MarketOrderFailureEvent(self._current_timestamp, order_id, order_type))
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        ...
        try:
            ...
            if order_status == 'cancelled' and cancelled_id == exchange_order_id:
                ...
                self.logger().info(f"The order {order_id} has already been filled on LitebitPro. No cancellation needed.")
                await self._update_order_status()  # We do this to correctly process the order fill and stop tracking.
                return order_id
        except IOError as e:
            if "order not found" in str(e).lower():
                # The order was never there to begin with. So cancelling it is a no-op but semantically successful.
                self.logger().info(f"The order {order_id} does not exist on LitebitPro. No cancellation needed.")
                self.c_stop_tracking_order(order_id)
                self.c_trigger_event(self.MARKET_ORDER_CANCELLED_EVENT_TAG,
                                     OrderCancelledEvent(self._current_timestamp, order_id))
                return order_id
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on LitebitPro. "
                                f"Check API key and network connection.{e}"
            )
        return None
        ...
        *required
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel order on LitebitPro. Check API key and network connection."
            )
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg=f"Could not fetch account updates on LitebitPro. "
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg=f"Could not fetch trading rule updates on LitebitPro. "
    ...
    cdef c_start_tracking_order(self,
        ...
        """
        self._in_flight_orders[client_order_id] = LitebitProInFlightOrder(
            client_order_id,
            None,
            trading_pair,
            order_type,
    ...
    cdef object c_get_order_size_quantum(self, str trading_pair, object order_size):
        ...
        *required
        ...
        # LitebitPro is using the min_order_size as max_precision
