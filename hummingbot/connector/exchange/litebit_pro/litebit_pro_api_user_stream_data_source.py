...
class LitebitProAPIUserStreamDataSource(UserStreamTrackerDataSource):
            ...
            try:
                async with websockets.connect(Constants.BAEE_WS_URL) as ws:
                    ...
                    auth_request: Dict[str, Any] = {
                        ...
                        "data": self._litebit_pro_auth.get_ws_auth_data()
                    }
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        ...
                        if event_type == 'updated':
                            ...
                            raise ValueError(f"LitebitPro Websocket message does not contain an event type - {diff_msg}")
            except asyncio.CancelledError:
                raise
            except Exception:
                self.logger().error("Unexpected error with LitebitPro WebSocket connection. "
    ...
    async def custom_ping(self, ws: websockets.WebSocketClientProtocol):
        """
        Sends a ping meassage to the LitebitPro websocket
        :param ws: current web socket connection
        """
